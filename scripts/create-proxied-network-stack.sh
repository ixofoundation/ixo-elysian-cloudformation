...
sed -i '' "s|%%NodeDid%%|$NODE_DID|" "$SCRIPTS_DIR/../parameters/ElysianProxiedNetworkStack.parameters.json"

aws cloudformation create-stack --stack-name Elysian-ELB-Proxy-Network-Stack$STACK_SUFFIX --template-body file://templates/ElysianProxiedNetworkStack.yaml  --parameters file://parameters/ElysianProxiedNetworkStack.parameters.json --profile default --region $TARGET_REGION --capabilities CAPABILITY_IAM
